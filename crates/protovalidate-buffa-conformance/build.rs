...
type Validators = [protoc_gen_protovalidate_buffa::scan::MessageValidators];

/// Split a dotted proto name into Rust path segments using buffa's nesting
/// convention: `PascalCase` segments that hold nested types lower to
/// `snake_case`, the leaf keeps its original case.
fn rust_segs(dotted: &str) -> Vec<String> {
...
/// One arm per message, dispatching to the owned struct under `crate::generated`.
fn write_owned_table(out: &mut String, validators: &Validators) {
    out.push_str(
        "pub(crate) fn dispatch_known(fqn: &str, bytes: &[u8]) -> Option<CaseOutcome> {\n",
    );
    out.push_str("    match fqn {\n");
    for v in validators {
        let path = format!("crate::generated::{}", rust_segs(&v.proto_name).join("::"));
        writeln!(
            out,
            "        \"{fqn}\" => Some(run::<{path}>(bytes)),",
            fqn = v.proto_name
        )
        .unwrap();
    }
...
/// The same set of messages, dispatching to their borrowed views.
/// `__buffa::view::` hangs off the package root, so the message-local segments
/// are re-rooted under it and the leaf gains the `View` suffix.
fn write_view_table(out: &mut String, validators: &Validators) {
    out.push_str(
        "pub(crate) fn dispatch_known_view(fqn: &str, bytes: &[u8]) -> Option<CaseOutcome> {\n",
    );
    out.push_str("    match fqn {\n");
    for v in validators {
        let local = v
            .proto_name
            .strip_prefix(&format!("{}.", v.package))
            .unwrap_or(&v.proto_name);
        let mut segs = rust_segs(local);
        segs.last_mut()
            .expect("proto name has at least one segment")
            .push_str("View");
        let path = format!(
            "crate::generated::{}::__buffa::view::{}",
            rust_segs(&v.package).join("::"),
            segs.join("::")
        );
        writeln!(
            out,
            "        \"{fqn}\" => Some(run_view::<{path}>(bytes)),",
            fqn = v.proto_name
        )
        .unwrap();
    }
...
fn write_dispatch_rs(out_dir: &std::path::Path, validators: &Validators) {
    ...
    write_owned_table(&mut out, validators);
    write_view_table(&mut out, validators);
    ...
    out.push_str("    };\n");
    out.push_str("    outcome(msg.validate())\n");
    out.push_str("}\n\n");

    out.push_str(
        "fn run_view<'a, V: ::buffa::MessageView<'a> + Validate>(bytes: &'a [u8]) -> CaseOutcome {\n",
    );
    out.push_str("    let view = match V::decode_view(bytes) {\n");
    out.push_str("        Ok(v) => v,\n");
    out.push_str("        Err(e) => return CaseOutcome::RuntimeError(format!(\"decode: {e}\")),\n");
    out.push_str("    };\n");
    out.push_str("    outcome(view.validate())\n");
    out.push_str("}\n\n");

    out.push_str(
        "fn outcome(r: Result<(), ::protovalidate_buffa::ValidationError>) -> CaseOutcome {\n",
    );
