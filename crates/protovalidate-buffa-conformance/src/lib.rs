//! Shared surface for the conformance executor: the generated message types,
//! the dispatch registry, and the conversion from this crate's violation types
//! back to the harness's own protos.
//!
//! Lives in a library rather than beside `main` so the benches can reach the
//! same generated types the executor validates.
