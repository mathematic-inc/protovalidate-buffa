        ...
        match &operand.ty {
            CelType::Map(map_ty) => {
                ...
                let (tokens, ty) = match val_ty {
                    CelType::Str { .. } => (
                        quote! {
                            ((#op_t).get(#key_lookup).map_or("", |__v| &__v[..]))
                        },
...
fn element_binding_expr(var_ident: &syn::Ident, elem_ty: &CelType) -> TokenStream {
    match elem_ty {
        CelType::Str { .. } | CelType::Bytes { .. } => quote! { (&#var_ident[..]) },
        CelType::Int => quote! {
...
fn select_message_field(
    ...
    let access = match &entry.kind {
        SchemaFieldKind::StringLike => match &entry.ty {
            CelType::Str { .. } | CelType::Bytes { .. } => {
                quote! { (&#operand.#rust_ident[..]) }
            }
        ...
        SchemaFieldKind::Wrapper => match &entry.ty {
            ...
            CelType::Str { .. } => {
                quote! { (#operand.#rust_ident.as_option().map_or("", |w| &w.value[..])) }
            }
            CelType::Bytes { .. } => {
                quote! { (#operand.#rust_ident.as_option().map_or(&[][..], |w| &w.value[..])) }
            }
